...
describe("caravanSeller", () => {
    ...
    it("no winner when both out of range (over)", () => {
        expect(
            caravanSeller(mkGame([caravanOf(["10", ["10", 1]])], [caravanOf(["10", ["10", 1]])]), 0)
        ).toBe(null);
    });
    it("higher in-range value wins", () => {
        expect(
            caravanSeller(mkGame([caravanOf([["10", 1], "4"])], [caravanOf(["10", "9"])]), 0)
        ).toBe(Human);
        expect(
            caravanSeller(mkGame([caravanOf(["10", "9"])], [caravanOf([["10", 1], "4"])]), 0)
        ).toBe(Ai);
    });
    it("tie in range is not resolved", () => {
        expect(
            caravanSeller(mkGame([caravanOf([["10", 1], "4"])], [caravanOf([["10", 1], "4"])]), 0)
        ).toBe(null);
    });
    it("both in range picks the higher", () => {
        expect(
            caravanSeller(mkGame([caravanOf([["10", 1], "4"])], [caravanOf(["10", "8", "4"])]), 0)
        ).toBe(Human);
    });
});
...
describe("allSold / gameWinner", () => {
    it("not all pairs resolved until every pair has winner", () => {
        ...
        expect(
            [0, 1, 2].every((laneIndex) => caravanSeller(g, laneIndex as 0 | 1 | 2) !== null)
        ).toBe(false);
    });
...
describe("gameWinner edges", () => {
    function twoSoldPlus(
        humanLane2: Array<string | [string, number]>,
        aiLane2: Array<string | [string, number]>
    ): GameState {
        return mkGame(
