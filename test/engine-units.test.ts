...
import { Caravan, CaravanRow, GameState, PlayerState, Human, Ai, Card } from "../src/model/types";

function rowHearts5(): CaravanRow { return [makeCard(1, "5", "hearts")]; }
...
describe("cloneAndApply", () => {
  it("clones without mutating original", () => {
...
describe("appendActionLog", () => {
  it("appends describe text to next.log", () => {
    ...
    expect(next.log[0].text).toMatch(/played/);
  });
});

describe("resolveTerminal", () => {
  it("flips turn when no winner", () => {
    const c: Caravan = { rows: [rowHearts5()], direction: null, suit: "hearts" } as unknown as Caravan;
...
describe("applyMove orchestration", () => {
  it("equals cloneAndApply+append+resolve", () => {
    resetLogIds();
