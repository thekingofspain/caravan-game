...
function mkPlayer(caravans: Caravan[], hand: Card[]): PlayerState {
    return {
        shoe: [],
        hand,
        caravans: caravans as [Caravan, Caravan, Caravan]
    } as unknown as PlayerState;
}
function mkGame(
    p0: PlayerState,
    p1: PlayerState,
    current: typeof Human | typeof Ai = Human
): GameState {
    return { players: [p0, p1], current, phase: "play", winner: null, log: [] };
}
function openValueSituation() {
    const g = mkGame(
        mkPlayer([caravanEmpty(), caravanEmpty(), caravanEmpty()], [makeCard(1, "5", "hearts")]),
        mkPlayer([caravanEmpty(), caravanEmpty(), caravanEmpty()], []),
        Human
    );
    return { g };
}
...
describe("cloneAndApply", () => {
    ...
    it("throws when game over", () => {
        ...
        g.phase = "gameOver";
        expect(() =>
            cloneAndApply(g, { type: "discardCard", player: Human, handIndex: 0 })
        ).toThrow(/game over/);
    });
});
...
describe("appendActionLog", () => {
    function loggedSituation() {
        ...
        const g = mkGame(
            mkPlayer([caravanEmpty(), caravanEmpty(), caravanEmpty()], hand),
            mkPlayer([caravanEmpty(), caravanEmpty(), caravanEmpty()], []),
            Human
        );
        const { next, jokerDetail } = cloneAndApply(g, {
            type: "playValueCard",
            player: Human,
            lane: 1,
            handIndex: 0
        ...
        appendActionLog(
            next,
            { type: "playValueCard", player: Human, lane: 1, handIndex: 0 },
            g,
            jokerDetail
        );
...
describe("resolveTerminal", () => {
    it("flips turn when no winner", () => {
        const c: Caravan = {
            rows: [[makeCard(1, "5", "hearts")]],
            direction: null,
            suit: "hearts"
        } as unknown as Caravan;
        const g = mkGame(
            mkPlayer([c, c, c], [makeCard(1, "2", "hearts")]),
            mkPlayer([c, c, c], [makeCard(1, "2", "hearts")]),
            Human
        );
