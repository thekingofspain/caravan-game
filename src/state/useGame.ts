...
import { getTransitionInfo, type TransitionInfo } from "../game/transition";
...
type ReducerAction = Action | { type: "reset"; config: GameConfig } | { type: "__setState"; state: GameState };
...
export function useGame(initial: GameConfig): GameStore {
  ...
  const [lastMove, setLastMove] = useState<Action | null>(null);
  ...
  const [pendingMove, setPendingMove] = useState<Action | null>(null);

  // Test harness: allow e2e to program particular hand/deck/ops
  if (typeof window !== "undefined") {
    const w = window as unknown as { __setCaravanState?: (s: GameState)=>void; __caravanDispatch?: (a: ReducerAction)=>void };
