...
async function stackSelectable(ci) {
  ...
  return (await stack.locator(".track.selectable").count()) > 0;
}
...
const ci = 1;
const before = await page.locator(".caravans.human .caravan").nth(ci).locator("button.card[data-index]").count();
...
for (let step = 0; step < 2; step++) {
  if (await addValueCardTo(ci)) {
    added++;
    const stack = page.locator(".caravans.human .caravan").nth(ci);
    const cardsNow = await stack.locator("button.card[data-index]").count();
...
const after = await page.locator(".caravans.human .caravan").nth(ci).locator("button.card[data-index]").count();
...
const wraps = page.locator(".caravans.human .caravan").nth(ci).locator("button.card[data-index]");
...
for (let i = 0; i < n; i++) {
  ...
  // The row button IS the card now (no wrapper): inspect the element itself.
  const isBack = await wraps.nth(i).evaluate((el) => {
    const bg = getComputedStyle(el).backgroundImage;
    return el.className.includes("back") || bg.includes("back.svg");
  });
