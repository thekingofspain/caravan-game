...
const aiHandJoker = [makeCard("spades","JOKER"), makeCard("hearts","7")];
...
let aiCardsBefore = await page.locator(".play-row--ai .caravan--ai .card").count();
...
let humanCardsBefore = await page.locator(".play-row--human .caravan--human .card").count();
let aiCardsBefore2 = await page.locator(".play-row--ai .caravan--ai .card").count();
...
let pending = await page.locator(".card.is-pending, .card.is-pending-remove").count();
let xCount = await page.locator(".jack-remove").count();
...
assert.ok(xCount>0, "jack-remove X should be visible (original git UX: X on Joker attachment)");
...
let selectableWhilePending = await page.locator(".hand__slot.is-selectable").count();
...
let humanCardsGrey = await page.locator(".play-row--human .caravan--human .card.is-pending").count();
let aiCardsGrey = await page.locator(".play-row--ai .caravan--ai .card.is-pending").count();
...
await page.locator(".jack-remove").first().click({ force:true });
await page.waitForTimeout(700);
let pendingAfter = await page.locator(".card.is-pending").count();
let humanAfter = await page.locator(".play-row--human .caravan--human").first().locator(".card").count();
let aiAfter = await page.locator(".play-row--ai .caravan--ai").first().locator(".card").count();
let totalAfter = await page.locator(".play-row--human .caravan--human .card, .play-row--ai .caravan--ai .card").count();
...
assert.ok(await page.locator(".hand__slot.is-selectable").count()>0, "human unblocked after ack");
...
await page.waitForTimeout(500);
let pendingJack = await page.locator(".card.is-pending").count();
let transJack = await page.evaluate(()=> window.__caravanStore.transition);
console.log(` after AI Jack: pending=${pendingJack} transition=${JSON.stringify(transJack)} x=${await page.locator(".jack-remove").count()}`);
...
assert.equal(await page.locator(".hand__slot.is-selectable").count(), 0, "human blocked for Jack ack");
await page.locator(".jack-remove").first().click({ force:true });
await page.waitForTimeout(600);
assert.equal(await page.locator(".card.is-pending").count(), 0, "Jack pending cleared");
assert.equal(await page.locator(".play-row--human .caravan--human").nth(1).locator(".card").count(), 0, "3c removed after Jack ack");
