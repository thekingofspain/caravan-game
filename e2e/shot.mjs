...
async function humanTurn(){
  ...
  const ready=page.locator(".side--human .caravan.is-selectable, .card.is-target").first();
...
for(let m=0;m<20;m++){
  ...
  try{await page.waitForSelector(".player-human .hand-zone .hand__slot.is-selectable",{timeout:5000});}catch{break;}
  await humanTurn();
...
const bad=await page.evaluate(()=>{
  const out=[];
  for(const el of document.querySelectorAll(".placed-face, .card .card, .placed .card")){
