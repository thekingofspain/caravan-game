...
async function humanTurn(){
  ...
  const ready=page.locator(".side--human .caravan.selectable, .card.target").first();
  ...
  else{const own=page.locator(".side--human .caravan.selectable").first();
...
for(let m=0;m<20;m++){
  ...
  try{await page.waitForSelector(".hand.human .slot.selectable",{timeout:5000});}catch{break;}
  await humanTurn();
...
const bad=await page.evaluate(()=>{
  const out=[];
  for(const el of document.querySelectorAll(".card, .card .card, .placed .card")){
